...
def main(dataset_filepath, h, algorithm, cost, gamma, model_filename):
	...
	outputs_filepath = 'CV_Results'
	if not os.path.isdir(outputs_filepath):
		os.mkdir(outputs_filepath)
	outputs_filename = 'CV_Results' +'.txt'
	fd = open(os.path.join(outputs_filepath, outputs_filename), 'w')
	...
	for index in range(1,11):
		...
		fold_outputs_filename = "fold" + str(index) + "_labels_test_" + ".txt"
		f = open(os.path.join(outputs_filepath, fold_outputs_filename) ,'w')
...
if __name__ == '__main__':
	...
	parser.add_option("-m", "--model_filename", dest="model_filename", help="outputs model filename", default='ClassifyTE.pkl')

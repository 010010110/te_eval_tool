...
# Este dicionário NÃO é mais usado para escrever a saída, apenas para o log
classification = {
...
with tf.compat.v1.Session(graph=tf.Graph()) as sess:
    ...
    for fl in options.files:
        ...
        for batch in range(0, test_size, options.batch):
            ...
            # (Correção de One-Hot que fizemos antes)
            one_hot_3d = (x_batch[..., None] == np.arange(vocab_size)).astype(np.float32)
            x_batch_one_hot = np.expand_dims(one_hot_3d, axis=-1)
        ...
        # --- INÍCIO DA CORREÇÃO ---
        # O 'classification' dict hardcoded estava causando o bug
        # Vamos usar um contador simples e escrever a predição real.
        pred_counter = {} 

        for i, pred_index in enumerate(predictions):
            # 1. Obter o nome da classe real que o modelo previu (ex: 'Copia', 'Zator')
            predicted_class_name = classes[pred_index]

            # 2. Atualizar o contador
            if predicted_class_name not in pred_counter:
                pred_counter[predicted_class_name] = 0
            pred_counter[predicted_class_name] += 1

            # 3. Escrever o NOME CURTO (a predição real) no cabeçalho
            out += '>' + predicted_class_name + '\t' + str(pred_counter[predicted_class_name]) + '\n'
            out += seqs[i]
        # --- FIM DA CORREÇÃO ---
        ...
        # --- INÍCIO DA CORREÇÃO (Impressão de Log) ---
        # Corrigir o loop de impressão para usar o novo pred_counter
        for cl, count in pred_counter.items():
             if options.verbose: print(f'{cl:>20s}:{count:7d}')
        # --- FIM DA CORREÇÃO ---

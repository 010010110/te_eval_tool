...
def feature_generation(curr_dir1,outputs_file, feature_dir):
	...
	kanalyzer_outputs_destpath = feature_destpath + "/kanalyze-2.0.0/outputs_data/"

	mer2_dir = kanalyzer_outputs_destpath + '2mer/'
	mer3_dir = kanalyzer_outputs_destpath + '3mer/'
	mer4_dir = kanalyzer_outputs_destpath + '4mer/'

	if os.path.isdir(mer2_dir):
		subprocess.run(['rm','-R',kanalyzer_outputs_destpath + '2mer'])
		os.mkdir(mer2_dir)
	else:
		os.mkdir(mer2_dir)

	if os.path.isdir(mer3_dir):
		subprocess.run(['rm','-R',kanalyzer_outputs_destpath + '3mer'])
		os.mkdir(mer3_dir)
	else:
		os.mkdir(mer3_dir)

	if os.path.isdir(mer4_dir):
		subprocess.run(['rm','-R',kanalyzer_outputs_destpath + '4mer'])
		os.mkdir(mer4_dir)
	else:
		os.mkdir(mer4_dir)
	...
	if not outputs_file == "feature_file.csv":
		subprocess.run(['mv', 'feature_file.csv', outputs_file])
...
def get_data(fasta_file, feature_dir):
	...
	if os.path.isdir(kanalyzer_outputs_destpath):
		subprocess.run(['rm','-R',kanalyzer_outputs_destpath])
		os.mkdir(kanalyzer_outputs_destpath)
	else:
		os.mkdir(kanalyzer_outputs_destpath)
...
def main():
	...
	parser.add_option("-o", "--outputs", dest="outputs_filename", help="Name of feature file", default="feature_file.csv")
	parser.add_option("-d", "--featuredir", dest="feature_dir", help="feature directory.", default="features")
	(options, args) = parser.parse_args()
	
	fasta_file = options.filename
	outputs_file = options.outputs_filename
	...
	feature_generation(curr_dir1, outputs_file, feature_dir)
